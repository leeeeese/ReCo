...
export default function ProductCard({
  ...
  return (
    <Card className={`overflow-hidden rounded-3xl hover:shadow-lg transition-shadow ${compact ? 'p-3' : 'p-0'}`}>
      <div className={compact ? 'flex gap-3' : ''}>
        <ImageWithFallback 
          src={image} 
          alt={name}
          className={compact ? 'w-20 h-20 rounded-2xl object-cover' : 'w-full h-48 object-cover'}
        />
        <div className={compact ? 'flex-1 min-w-0' : 'p-5'}>
          ...
          <div className={`flex items-baseline gap-2 mb-2 ${compact ? 'text-sm' : ''}`}>
            <span className="text-emerald-600">{price}</span>
            {avgPrice && (
              <>
                <span className="text-gray-400 line-through text-sm">{avgPrice}</span>
          ...
          {reason && !compact && (
            <p className="text-sm text-gray-600 mb-4 line-clamp-2">{reason}</p>
          )}
