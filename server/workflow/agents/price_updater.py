...
def serp_search(query: str, max_results: int = 30) -> List[Listing]:
    ...
    for item in data.get("shopping_results", []):
        p = item.get("extracted_price") or _parse_prices_from_texts(
            [item.get("price", "")])
        if p:
            out.append(Listing(item.get("title", ""), item.get(
                "link", ""), "shopping", float(p if isinstance(p, float) else p[0])))
    for item in data.get("organic_results", []):
        txt = item.get("title", "") + " " + item.get("snippet", "")
        prices = _parse_prices_from_texts([txt])
        if prices:
            out.append(Listing(item.get("title", ""), item.get(
                "link", ""), "organic", prices[0]))
    return out
...
class DB:
    ...
    def update_item_pricing(self, item_id: int, metrics: Dict[str, Any]):
        ...
        self.conn.execute(q, (metrics.get("used_avg"), metrics.get("used_p50"),
                              metrics.get("discount_vs_used_avg"), metrics.get(
                                  "discount_vs_used_p50"),
                              now, item_id))
